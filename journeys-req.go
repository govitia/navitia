...
const isoDateTimeFormat string = "20060102150405" // YYYYMMDDThhmmss
...
type JourneyRequest struct {
	...
	FirstSectionModes []Mode

	// Same, but for the last section
	LastSectionModes []Mode
...
func (req JourneyRequest) toURL() (url.Values, error) {
	...
	addIDSlice := func(key string, ids []ID) {
		if len(ids) != 0 {
			for _, id := range ids {
	...
	addModes := func(key string, modes []Mode) {
		if len(modes) != 0 {
	...
	if datetime := req.Date; !datetime.IsZero() {
		str := datetime.Format(isoDateTimeFormat)
